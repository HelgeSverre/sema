...
;; Hand-rolled integer×10 parser: parse "[-]ddd.d" -> temp×10 (integer).
;; Matches the Chicken/Guile fast path instead of generic string->number.
...
;; Format an integer temperature ×10 as "whole.frac" with one decimal place.
(define (format-1dp n)
  (let* ((abs-n (abs n))
