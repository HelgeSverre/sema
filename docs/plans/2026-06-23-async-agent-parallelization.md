...
>
> **Adversarial pass earned its keep (the §5 narrow gate was insufficient):** the first cut parked-on-IO unconditionally, which (#2) starved sleepers and (#3) silently disabled `async/timeout` over in-flight IO. Fixed (`b3d513e`): the park advances `virtual_now` by **real elapsed**, bounded by the nearest sleeper/timeout deadline and capped at 50 ms for interrupt cadence, and replicates the `sleep_limit()` timeout check — so sleepers + timeouts stay live while IO is in flight, B3 included. Both repros added as TDD gates (failed pre-fix, pass post-fix). — **§8 Verification addendum still governs Phase 2 (B1–B5).**
...
> - **✅ #4a concurrent `llm/embed` + per-task OTel** — SHIPPED & verified. `llm/embed` is a first-class native (`(procedure? llm/embed)`→#t; `(async/pool-map llm/embed chunks N)`→real concurrent embeds, dims 8192×N), branching on `in_async_context()`; sync path byte-identical. Per-task OTel TLS context-switch (panic-safe in `ReinstallGuard`) + detached span carried in the `IoHandle` poller (which also runs `track_usage`); Arc'd provider registry + `spawn_blocking`. **Adversarial pass caught 2 defects, both fixed before merge:** (i) the first cut made `llm/embed` a router *macro* (broke `pool-map`/`map`/`procedure?`) → collapsed to a single native (`b2e87cb`); (ii) an in-flight embed abandoned by `async/timeout`/`async/cancel` under OTel left a stranded un-finalized span that aborted at TLS teardown → scheduler now **reaps leftover tasks at the outermost run exit** (`4bf0337`; gate: repro exits clean 5×, `scheduler_task_count` 1→0). Verified: overlap 360 ms vs 600 ms serial; distinct isolated spans (12-way); conversation-scope inheritance; live OpenAI test; `cargo test --workspace` 88 ok / 0 failed. **`classify`/`extract`/`complete` (multi-yield tool loop) reuse the per-task OTel but need a multi-span carry — deferred to a separate plan.**
>   - **⚠️ Finding (verified 2026-06-23, harder than §8 framed B2):** `llm/embed` (`builtins.rs:2636`) holds the otel `SpanCore` (an RAII guard) as a **local of the native frame** across the provider call (`let span = llm_span("embeddings")` … `span.set_response(...)`). But a native fn's locals **drop when it yields** (the frame returns to the scheduler on `set_yield_signal`), so the span would close at the yield with no response recorded — *and* B2's TLS-stack snapshot doesn't help because the guard lives in the frame, not just the TLS. So a faithful async otel span needs the span handle **stored per-task** (in the `Task` or a Value-wrapped handle finalized by a native on resume), which is a bigger change than a TLS swap.
>   - **v1 scoping (recommended):** ship concurrent `llm/embed` via `spawn_blocking(provider.embed(req))` (B1; Arc the embedding registry) with cassette/usage on the VM thread (cassette check pre-spawn; `track_usage` on resume) and **defer the async-path otel span** (sync embeddings keep full otel; concurrent embeddings are untraced in v1 — documented gap). Delivers the RAG-indexing value without the per-task-span work. Gate: concurrent embeds overlap (live, keys in env) + correctness + sync path byte-identical.
...
> - **#4b concurrent `llm/complete` / `classify` / `extract`** + **#5 true cancel** — now spec'd in a focused follow-on plan: **`docs/plans/archive/2026-06-23-concurrent-complete-and-true-cancel.md`**. Key de-risk: offload the whole retry+fallback unit via `spawn_blocking(provider.complete)` (the embed precedent) — reuses the sync path, so B1 + the DROP_TEMPERATURE / retry-drift majors **dissolve**. True cancel adds an abort seam to `IoHandle` (real `AbortHandle`/`kill_on_drop` for the `spawn`-based http/shell; honest best-effort + round-cutoff for the `spawn_blocking` LLM tier).
> - **⚠️ Regression fixed (`0b08e0f`, caught by `make examples` — plain `cargo test` skips it):** the adversarial-#7 reap cleared **all** leftover tasks at the outermost scheduler exit, on the false premise that any task held there is abandoned. A task spawned before an `(async/all …)` and awaited after it (streaming-pipeline `collector`) was wrongly reaped → "async/await: still pending after scheduler run" (`async-pipeline`/`async-stress`). Fix: reap **terminal-only**; `async/timeout` expiry now `cancel_promise_task`s its target so the abandoned span still finalizes while TLS is alive (`scheduler_task_count` 1→0 preserved). Deterministic regression test added.
>
