...
> - **#4 single-shot LLM** (`llm/embed` first → `classify`/`extract`/`complete` — B1 via `spawn_blocking`, B2, thread-local capture; no B4, no loop lift). Unblocks RAG indexing, batch classify/extract.
