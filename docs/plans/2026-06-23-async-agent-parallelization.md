...
> - **#4b concurrent `llm/complete` / `classify` / `extract`** + **#5 true cancel** — now spec'd in a focused follow-on plan: **`docs/plans/2026-06-23-concurrent-complete-and-true-cancel.md`**. Key de-risk: offload the whole retry+fallback unit via `spawn_blocking(provider.complete)` (the embed precedent) — reuses the sync path, so B1 + the DROP_TEMPERATURE / retry-drift majors **dissolve**. True cancel adds an abort seam to `IoHandle` (real `AbortHandle`/`kill_on_drop` for the `spawn`-based http/shell; honest best-effort + round-cutoff for the `spawn_blocking` LLM tier).
