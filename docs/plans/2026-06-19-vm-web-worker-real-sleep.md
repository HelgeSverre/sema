...
- **M6 — Soft cancellation + revived runaway guard. ✅ DONE 2026-06-19.** Discovery: the VM had **no live interrupt point** for normal eval (`eval_step_limit`/`eval_steps` were dead tree-walker leftovers; the `deadline` mechanism used `Instant`, which panics in wasm and was never set), so the playground's "10M-step guard" was inert and runaway loops froze the tab. Fix (chosen: add a VM interrupt + revive the guard):
  - **`EvalContext::check_loop_interrupt()`** — one unified guard called at the VM's loop back-edges (`JUMP`/`JUMP_IF_FALSE`/`JUMP_IF_TRUE` backward) and the outer dispatch (tail-call) transition. Per call: bump `eval_steps`, fail past `eval_step_limit` (wasm-safe runaway guard); every 16384 steps also check the wall-clock deadline (native) and the interrupt callback (cancellation). `eval_steps` reset per top-level eval.
  - **`sema_core::set_interrupt_callback`/`check_interrupt`** — a `fn() -> bool` the host installs. The worker's `worker_check_interrupt` reads control-SAB slot 0 (`Atomics.load`); a cancel stores 1 + `Atomics.notify`, which both wakes a real `Atomics.wait` sleep early and trips the guard. Scheduler loop also checks it between task steps and **clears pending tasks** on cancel (no dangling/sleeping tasks resurrect later).
  - **UI:** the Run button becomes **Stop** during a worker run (`cancelWorker()` = store+notify); status shows Running…/Stopped/Error.
  - *Accept — PASSED:* native tests `step_limit_aborts_runaway_loop` + `interrupt_callback_cancels_evaluation`; e2e: Stop cancels a 5s real-sleep in <3s, status "Stopped", and a **subsequent run still works (worker survives, state preserved)**. Full native suite + e2e 58/58 green; clippy + fmt clean. Bonus: the revived step-limit guard fixes the pre-existing tab-freeze on runaway loops (main-thread path).
