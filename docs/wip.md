...
### L3 — DAP conditional / exception breakpoints
`supportsConditionalBreakpoints` / `supportsExceptionInfo` are still `false` in `crates/sema-dap`. (`setVariable`, `evaluate`/hover-eval already shipped.)
